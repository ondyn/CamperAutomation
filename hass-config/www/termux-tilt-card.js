...
class TermuxTiltCard extends HTMLElement {
  ...
  static getStubConfig() {
    return {
      ...
      measure_button_entity: "button.van_tilt_meter_sample_once",
      reset_button_entity: "button.van_tilt_meter_reset_zero",
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .car::before {
          ...
          bottom: 14%;
          border-radius: 12px;
          background: linear-gradient(180deg, rgba(148, 163, 184, 0.26), rgba(51, 65, 85, 0.08));
        }
      ...
      <ha-card>
        ...
        <div class="buttons">
          ...
          <button id="reset" class="secondary">Reset zero</button>
        </div>
    ...
    this.shadowRoot.getElementById("measure")?.addEventListener("click", () => {
      this._callService("button", "press", this._config.measure_button_entity);
    });

    this.shadowRoot.getElementById("reset")?.addEventListener("click", () => {
      this._callService("button", "press", this._config.reset_button_entity);
    });
  }
}
...
window.customCards.push({
  ...
  description: "Visualizes wheel lift heights in cm with quick measurement controls.",
});
